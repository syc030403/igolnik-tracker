...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    title: {
      absolute: `${dict.metaCatTitle.replace("{cat}", label).replace("{mode}", "PVE")} | Igolnik Tracker`,
    },
    description: dict.metaCatDesc.replace("{cat}", label).replace("{mode}", "PVE"),
    alternates: {
      canonical: localePath(locale, `/market/pve/${category.slug}`),
      languages: Object.fromEntries([
        ...LOCALES.map((l) => [l, localePath(l, `/market/pve/${category.slug}`)]),
        ["x-default", `/market/pve/${category.slug}`],
      ]),
    },
  };
}

export default async function MarketPveCategoryPage({ params }: Params) {
  const { lang, cat } = await params;
  const locale: Locale = isLocale(lang) ? lang : "ko";
  const dict = getDict(locale);
  const category = findCategory(cat);
  ...
  try {
    items = await getMarketItemsByCategory(category, "pve", locale);
  } catch {
    items = null;
  }
  ...
  return (
    <>
      ...
      <main className={styles.main}>
        ...
        {items ? (
          <MarketView items={items} mode="pve" cat={category.slug} />
        ) : (
